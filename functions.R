...
lagphase <- function(data, knots=NULL, order=1, gam=FALSE)
{
    ...
    if(length(knots)==1)
    {
      if(knots==0) # i.e., no knots to be included
      {
        ...
        if(!is.null(data$island))
          fit$name <- paste(data$species)
        else
          fit$name <- paste(data$species)#deparse(substitute(data))
        fit$data <- data
        return(fit)
      }
    }
  ...
  if(AICc(fit0) < bestfit$value)
  {
    ...
    if(!is.null(data$island))
      bestfit$name <- paste(data$species)
    else
      bestfit$name <- paste(data$species)#deparse(substitute(data))
    bestfit$data <- data
  }
