...
lagfit = function(data, ydata, zeros=TRUE){ 
  ...
  for(island in c("North", "South")) { #todo: use unique(islands) to cater for varying descriptions
    
    idata = subset(data, data$Island == island)
    Species = unique(as.character(idata$Species))
    
    # Run the main loop
    for(i in 1: length(Species)) {
      sdata = get.species(idata,
                          ydata,
                          island = island,
                          species = Species[i],
                          zeros = zeros)
  ...
  out = data.frame(Island = outisland, Species = outspecies, Scene = outscene, Lag =outlag, Laglength = outlaglength, FirstYear=outfirstyear, EndYear=outendyear)
  out
}
...
print.lagphase <- function(x,...) {
  if (x$lagphase) {
    cat(paste(
      ...
      x$data$Year[1],
      "-",
      round(x$data$Year[1] + x$lengthlag),
      " (",
      round(x$lengthlag),
      " Years)\n",
      sep = ""
